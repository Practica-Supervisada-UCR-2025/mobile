...
final String _apiKey = dotenv.env['TENOR_API_KEY'] ?? '';
  ...
  Future<List<GifModel>> searchGifs(String query, {int limit = 20}) async {
    final response = await http.get(Uri.parse(
      '$_baseUrl/search?q=$query&key=$_apiKey&client_key=$_clientKey&limit=$limit&media_filter=tinygif',
    ));
  ...
  Future<List<GifModel>> getTrendingGifs({int limit = 20}) async {
    final response = await http.get(
      Uri.parse(
        '$_baseUrl/featured?key=$_apiKey&client_key=$_clientKey&limit=$limit&media_filter=tinygif',
      ),
      headers: {
